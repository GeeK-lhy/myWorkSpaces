...
<script type="text/javascript">
 $(function(){
	$("#select1").change(function(){
		$("#text1").load("getChargeByCode",{"charge_Code":$("#select1").val()},function(data){
			$("#text1").val(data)
		});
	});
	 $("#btn1").click(function(){
		 $.get("updateCharge",{"charge":$("#text1").val(),"charge_Code":$("#select1").val()},function(){
			 alert("收费明细提交成功")
		 });
	 });
	 $("#btn2").click(function(){
        ...
        $("input[name='check']:checked").each(function(i){
     	...
     	var func_Id=$("#select2").val();
/* 		 $.post("calculate",{"checkId":checkId,"func_Id":$("#select2").val(),"func_Name":"开户业务"},function(data){
			 alert(data);
		 }); */
		 $.ajax({
			    url: "calculate",
    ...
    <TD width=173 bgColor=#c9d3e9 rowSpan=2>
      <TABLE cellSpacing=0 cellPadding=0 width=173 border=0>
        ...
        <TR>
          ...
          <TD width=165 height="29" bgColor=#7691c7>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<A 
            class=nav 
            href="/newProject/login.jsp">退出</A></TD>
        </TR>
        <TR>
          <TD bgColor=#ffffff colSpan=2><IMG height=1 
            src="images/blank.gif" width=1 border=0></TD></TR>
...
<h2>收费细项</h2>
    <form method="POST" action="--WEBBOT-SELF--">
      <p>收费项目：<select size="1" name="D1" id="select1">
       <c:forEach items="${charges}" var="c" varStatus="state">
...
<!--        <option>开户费</option>
      ...
      <p>收费金额：<input type="text" id="text1" name="T1" size="20" value="${charges[0].charge }"></p>
      <p><input type="button" id="btn1" value="提交" name="B1"><input type="reset" value="全部重写" name="B2"></p>
    </form>
    <hr>
    <h2>业务收费</h2>
    <form method="POST" action="--WEBBOT-SELF--">
      <p>业务：<select size="1" name="D1" id="select2">
        <option value="O">开户业务</option>
      </select></p>
      <p>收费项目：
       <c:forEach items="${charges}" var="c" varStatus="state">
       <c:choose>
       <c:when test="${c.charge_Code=='B'}">
   	 <input type="checkbox" name="check" value="${c.charge_Code}" >${c.charge_Name }
   	 </c:when>
   	 <c:otherwise>
   	 <input type="checkbox" checked name="check" value="${c.charge_Code}" >${c.charge_Name }
   	 </c:otherwise>
   	 </c:choose>
      </c:forEach> 
<!--       <input type="checkbox" name="C1" value="ON" checked>开户费 
      ...
      <p><input type="button" id="btn2" value="提交" name="B1"><input type="reset" value="全部重写" name="B2"></p>
    </form>   
