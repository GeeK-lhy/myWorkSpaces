...
import com.fh.service.system.gradehandler.impl.GradeHandlerService;
import com.fh.service.system.loginimg.LogInImgManager;
import com.fh.service.system.menu.MenuManager;
import com.fh.service.system.record.RecordManager;
...
public class LoginController extends BaseController {
			...
			if(null == code || "".equals(code)){//判断效验码
				...
				if(Tools.notEmpty(sessionCode) && sessionCode.equalsIgnoreCase(code)){		//判断登录验证码
					...
					if(pd != null){
						...
						user.setGRADENAME(pd.getString("GRADENAME"));
						user.setSTATUS(pd.getString("STATUS"));

						PageData pd2 = new PageData();
						pd2.put("GRADENAME", pd.getString("GRADENAME"));
						pd2=gradehandlerService.getdegree(pd2);
						
						
						PageData pd5 = new PageData();
						pd5.put("FLAG", "des中");
						session.setAttribute("record", pd5);
						List<PageData> glist=gradehandlerService.listAll(pd2);
						session.setAttribute("glist", glist);
						
						session.setAttribute(Const.SESSION_USER, user);			//把用户信息放session中
						session.setAttribute("degree", pd2.get("degree"));
	...
	public ModelAndView logout(HttpSession session) throws Exception{
		...
		PageData pd2=(PageData) session.getAttribute("record");
		if(pd2.getString("FLAG").equals("值班中")) {
			pd2.put("ENDTIME", new Date());
			pd2.put("FLAG", "下班");
			recordService.edit(pd2);
		}
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		this.removeSession(USERNAME);//请缓存
