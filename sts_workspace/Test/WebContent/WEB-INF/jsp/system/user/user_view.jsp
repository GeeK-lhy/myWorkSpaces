								...
								<form action="user/${msg }.do" name="userForm" id="userForm" method="post">
									...
									<table id="table_report" class="table table-striped table-bordered table-hover">
										...
										<%--  <c:forEach items="${roleList}" var="role"> --%>
											<%--  <option value="${role.ROLE_ID }" <c:if test="${role.ROLE_ID ==pd.ROLE_ID }">${role.ROLE_ID}</c:if> ${role.ROLE_NAME } </option>  --%>
											${role.ROLE_NAME }
							<%-- 				</c:forEach> 
											</select>  --%>
											</td>
										...
										<tr>
											...
											<td><input type="text" name="USERNAME" id="loginname" value="${null==pd?'无此用户名':pd.USERNAME }" maxlength="32" title="用户名" style="width:98%;" disabled="disabled"/></td>
										</tr>
										<tr>
